...
export const PendingReviewsTable = ({ reviews, reviewers, onAssign }) => {
    ...
    const [selectedReviewer, setSelectedReviewer] = useState({ id: '', name: '' });
    const [selectedTime, setSelectedTime] = useState('');

    const handleAssign = () => {
        if (!selectedReviewer || !selectedTime) {
            toast.err('Please select both reviewer and time');
            return;
        }
        console.log(selectedReviewer)
        onAssign(selectedReview._id, selectedReviewer.id, selectedReviewer.name, selectedTime);
                        ...
                        <tr key={review._id}>
                            <td className="px-6 py-4 whitespace-nowrap">
                                <div className="flex items-center">
                                    ...
                                    <div>
                                        <p className="font-medium">{review.studentId.fullname}</p>
                            ...
                            <td className="px-6 py-4">
                                <div className="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500">Week {review.week + 1}</p>
                                    </div>
                                </div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                                <div className="flex items-center">
                                    ...
                                    <span className={review.reviewerName ? "" : "text-gray-400"}>
                                        {review.reviewerName
                                            ? `${review.reviewDate.split('T')[0]}  ${review.time ? review.time : ''}  to
                                                ${review.reviewerName}
                                            `
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                                <span className={`px-2 py-1 rounded-full text-xs ${review.status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                    ...
                    <div className="bg-white rounded-lg p-6 w-full max-w-md">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <select
                                    value={JSON.stringify(selectedReviewer)}
                                    onChange={(e) => {
                                        const reviewer = JSON.parse(e.target.value);
                                        setSelectedReviewer(reviewer);
                                    }}
                                    className="w-full p-2 border rounded"
                                >
                                    <option value={JSON.stringify({ id: '', name: '' })}>Select reviewer</option>
                                    {reviewers.map((reviewer) => (
                                        <option
                                            key={reviewer._id}
                                            value={JSON.stringify({ id: reviewer._id, name: reviewer.fullname })}
                                        >
                                            {reviewer.fullname} ({reviewer.certificates})
                                        </option>
                                    ))}
                                </select>
                            </div>
